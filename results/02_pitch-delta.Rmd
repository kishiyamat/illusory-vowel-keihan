...
results_by_subject %>%
  mutate(
    ...
    x_tokyo_kinki_ratio =  factor(range01(span_tokyo_span_kinki/age),5 ),
    x_tokyo_other =  factor(range01(span_tokyo_span_kinki),5),
    x_tokyo_other_ratio =  factor(range01(span_tokyo_span_kinki/age),5 ),
    x_tokyo = factor(range01(span_tokyo),5),
    x_tokyo_ratio = factor(range01(span_tokyo/age),5),
  ) %>% 
  pivot_longer(cols = starts_with("x_"), names_to="x_name", values_to="x_value") %>% 
  group_by(phoneme, pitch_from, subj_id) %>% 
  ggplot() +
  facet_grid(x_name~phoneme+pitch_from) +
  geom_violin(aes(x=x_value, y=to_tokyo_pattern, color=x_name)) +
  geom_boxplot(aes(x=x_value, y=to_tokyo_pattern, color=x_name))
