...
```{r}
plot(model, variable = c("b_tokyo_kinki_ratio", "b_phonemeetsto", "b_pitch_fromFromL_H")) 
```
```{r}
...
results_by_subject %>% 
  ggplot() +
  facet_grid(pitch_from~phoneme) +
  geom_violin(aes(x=factor(range01(devoiced), 5),
                  y=to_tokyo_pattern, color=pitch_from, fill=pitch_from))+
  theme(legend.position="none") +
  labs(title="Preference to Tokyo Pattern",
       x ="Devoiced",
...
ggsave(paste0("artifact/preference_by_devoiced.png"), unit = "cm", width = 8, height = 8)
```
