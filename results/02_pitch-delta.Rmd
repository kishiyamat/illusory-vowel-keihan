...
# factor が わかりづらくしてる
factor_str = function(x, res) {as.character(as.integer(floor(x*res)))}
...
results_by_subject %>%
  ...
  select(subj_id, age, is_tokyo) %>% distinct() %>% 
  group_by(is_tokyo) %>% 
...
model_stan <- stan_lmer(
  to_tokyo_pattern ~ tokyo_kinki_ratio + (1 | item_id),
  data = (results_by_subject %>% filter(!subj_id %in% less_accurate_subj_id)),
...
```{r}
print(summary(model_stan, digits = 3))
```

```{r}
# 片側検定
bayesfactor_parameters(model_stan, null = c(-Inf, 0))
```
