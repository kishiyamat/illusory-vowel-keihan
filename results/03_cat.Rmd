...
item_list = read_csv("../src/list/cat_list.csv") %>%
  rowwise() %>% 
  mutate(wav_base = str_split(audio, "\\.")[[1]][1],
          phonene = str_split(wav_base, "-")[[1]][1],
          speaker = str_split(wav_base, "-")[[1]][2])
item_list

# item_list <- c("soundA_1.wav", "soundA_2.wav")
test <- data.frame(sound=sound)
test
...
results <- rbind(
  ...
  select(c(run_id, item_id, response, rt, span_kinki, span_tokyo, span_unknown, age, data_src)) %>% 
  drop_na() %>%
  mutate(
