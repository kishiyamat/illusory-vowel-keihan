...
library(lmerTest)
model <- glmer(
  rate / 6 ~ stim_devoiced * dev_env * tokyo_kinki_ratio + (1 | subj_id),
  data = results,
  family = binomial
)
summary(model)
...
results_match <- results %>%
  filter(choice == supposed_answer) %>%
  pivot_wider(
    id_cols = c(subj_id, tokyo_kinki_ratio, pair, spkr, dev_env),
...
library(lmerTest)
model <- lmer(
  rate_stim_dev_match_odds  ~ tokyo_kinki_ratio + (1 | subj_id),
