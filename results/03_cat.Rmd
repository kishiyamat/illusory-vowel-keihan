...
results <- results_cat %>%
  ...
  filter(type == "target") %>%
  filter(age > 18) %>%
  select(c(subj_id, tokyo_kinki_ratio, choice, rate, item_id)) %>%
  drop_na() %>%
  merge(item_list, on = "item_id") %>%
  mutate(
    stim_has_u = str_detect(stim, "u"),
...
```{r}
results
  results %>% filter(choice==supposed_answer) %>% ggplot() +
...
library(lmerTest)
model <- lmer(
  rate ~ stim_has_u * dev_env * tokyo_kinki_ratio + (1 | subj_id),
  data = results
)
summary(model)
```
