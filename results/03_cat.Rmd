...
results <- results_cat %>%
  ...
  select(c(run_id, item_id, response, rate, span_kinki, span_tokyo, span_unknown, age, data_src, type, choice, rate)) %>%
  drop_na() %>%
  mutate(
...
```{r}
# rateを可視化
results %>% group_by(phoneme) %>% 
  summarise(rate=mean(rate)) %>% print
