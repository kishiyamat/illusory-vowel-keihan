...
results_mean_by_item %>%
  ggplot() +
  facet_wrap(. ~ item_id) +
  geom_point(aes(x = span_kinki / age, y = voiced))

# tokyoの在住歴
results_mean_by_item %>%
  ggplot() +
  facet_wrap(. ~ item_id) +
  geom_point(aes(x = span_tokyo / age, y = voiced))

# dominance
results_mean_by_item %>%
  ggplot() +
  facet_wrap(. ~ item_id) +
  geom_point(aes(x = span_kinki - span_tokyo, y = voiced))

# dominance/年齢
results_mean_by_item %>%
  ggplot() +
  facet_wrap(. ~ item_id) +
  geom_point(aes(x = (span_kinki - span_tokyo) / age, y = voiced))
```
...
results_mean_by_item %>%
  ggplot() +
  geom_point(aes(x = span_kinki - span_tokyo, y = voiced, color=as.factor(item_id)))
```
