...
x_var_list <- c("span_tokyo", "span_kinki", "span_tokyo_span_kinki", "span_tokyo_span_other", "span_kinki_span_tokyo", "span_kinki_span_other")
...
for (x_var in x_var_list) {
  ...
  pivot_longer(x_var_list, names_to = "span", values_to = "year")%>% 
    ...
    filter(span == x_var) %>% mutate(devoiced_mean=mean(devoiced))%>% 
    ungroup() %>%
...
```
試しに `span_tokyo_span_kinki` を可視化

```{r}
# 可視化のため平均を取る
x_var = "span_tokyo_span_kinki"
result_x_var <- results %>%
  pivot_longer(x_var_list, names_to = "span", values_to = "year")%>% 
  filter(span == x_var) %>%
  group_by(subj_id, item_id) %>% 
